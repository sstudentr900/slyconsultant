...
class FnController extends Controller
{
    ...
    public function fnsearch(Request $request) {
      ...
      //post 傳值
      //有0移除
      if(substr($input['birth'], 0, 1) === "0"){$input['birth'] = substr($input['birth'],1);}
    ...
    public function fnconsult(Request $request){
      ...
      Mail::send(
        'email_consult', 
        ['name' => $input['name'],'email' => $input['email'],'tel' => $input['tel'],'textarea' => $input['textarea']], 
        function($mail) use ($input){
          // $mail->to('sstudentr900@gmail.com');
          // $mail->subject('來自馨琳揚企管顧問-'.$input['name'].'諮詢');
          $mail->from(env('MAIL_FROM_ADDRESS','service@sly-ha.com.tw'), '來自馨琳揚企管顧問');
          $mail->to(env('MAIL_FROM_ADDRESS','sstudentr900@gmail.com'));
          $mail->subject('來自馨琳揚企管顧問-'.$input['name'].'諮詢');
        }
      );
